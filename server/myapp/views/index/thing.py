...
def list_api(request):
    ...
    if request.method == 'GET':
        ...
        if keyword:
            things = (
                Thing.objects.filter(title__contains=keyword)
                .filter(status=tatus)
                .order_by(order)
            )
        elif username:
            # user = User.objects.get(username=username)
            things = Thing.objects.filter(recommend_count=username).order_by(order)
        # todo
        elif c and int(c) > -1:
            ids = [c]

            things = (
                Thing.objects.filter(classification_id__in=ids)
                .filter(status=tatus)
                .order_by(order)
            )
            ...
            print(tag)
            things = tag.thing_set.all().filter(status=tatus).order_by(order)
        else:
            things = (
                Thing.objects.all().filter(status=tatus).defer('wish').order_by(order)
            )
