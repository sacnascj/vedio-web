...
def count(request):
    if request.method == 'GET':
        ...
        # if keyword :
        sql_str = (
            "select id, title, pv as count from b_thing  order by pv desc limit 10 "
        )
        if keyword:
            sql_str = f"select id, title, pv as count from b_thing where recommend_count='{keyword}'  order by pv desc limit 10 "
        ...
        # 统计分类比例(sql语句)
        sql_str = (
            "select B.title, count(B.title) as count from b_thing A join B_classification B on "
            "A.classification_id = B.id group by B.title order by count desc limit 5; "
        )
        if keyword:
            sql_str = "SELECT   v.id AS video_id,  v.collect_count AS c_collect,  v.wish_count AS c_like, v.pv as c_watch ,  (SELECT COUNT(*) FROM b_comment c WHERE c.thing_id = v.id ) AS c_comment  FROM   b_thing v  WHERE  v.recommend_count='xlx'  ;"
        with connection.cursor() as cursor:
            cursor.execute(sql_str)
