...
def list_api(request):
    if request.method == 'GET':
        ...
        staus = request.GET.get("staus", None)
        ...
        if keyword:
            ...
            things = classification.classification_thing.all().filter(status=staus)
        elif tag:
            tag = Tag.objects.get(id=tag)
            print(tag)
            things = tag.thing_set.all()
        else:
            things = Thing.objects.all().filter(status=staus).order_by('-create_time')
